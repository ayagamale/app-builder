...
import { Plus, Trash2, Loader2, KeyRound, Activity, Zap } from "lucide-react";
...
export default function ApiKeysPage() {
  ...
  const fetchAll = () => {
    Promise.all([fetch("/api/admin/api-keys").then(r => r.json()), fetch("/api/admin/models").then(r => r.json()), fetch("/api/admin/providers").then(r => r.json())])
      .then(([k, m, p]) => { if (k.ok) setKeys(k.data); if (m.ok) setModels(m.data); if (p.ok) setProviders(p.data); if (m.ok && !form.model_id) setForm(f => ({ ...f, model_id: m.data[0]?.id || "" })); })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">API Credentials</h1><p className="text-sm text-gray-500 mt-1">Manage API keys — encrypted at rest, never shown in full</p></div>
      ...
      <div className="space-y-2">
        {keys.map((k) => (
          <div key={k.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center shrink-0"><KeyRound className="w-5 h-5 text-gray-500" /></div>
